...
| [`security-groups.png`](security-groups.png) | 직접 지정한 5개와 EKS가 자동 생성한 5개. 설명 열에 용도를 적어 두었다 | 앞의 5개는 AWS CLI, 나머지는 eksctl·ALB Controller |
...
| 화면 | 경로 | 백엔드 |
|---|---|---|
| [`site-home.png`](site-home.png) | `/` | ALB → ASG 인스턴스 → nginx 컨테이너. **「DB 데이터 가져오기」** 를 누르면 `/loadlist/items` 로 가서 nginx → fastAPI → 컨테이너 MySQL 을 거친다 |
...
| [`site-student.png`](site-student.png) | `/student` | 화면 자체는 S3 정적 웹사이트. **검색 버튼이 `/student/list` 를 호출**하면 Lambda → RDS Proxy → RDS 로 조회한다 |
...
**내부 통신 구간은 IP 대신 보안 그룹 참조로 구성했습니다.** ASG가 인스턴스를 교체하거나 파드 IP가 바뀌어도 규칙을 고칠 필요가 없습니다. IP 기반 규칙이 남는 곳은 두 군데뿐입니다 — 인터넷을 받아야 하는 ALB의 80/443, 그리고 SSH 접속을 허용해야 하는 Bastion 입니다.
