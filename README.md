...
`app.totorosi.cloud` 는 API Gateway를 거치지 않고 ALB로 바로 들어가는 경로입니다. ALB의 HTTPS 리스너와 인증서를 API Gateway와 분리해 단독으로 검증하려고 두었습니다.
...
**`ec2/nginx/default.conf` · `eks-app/nginx/default.conf`** — fastAPI를 외부에 열지 않고 nginx 리버스 프록시로만 접근시킵니다. 컨테이너는 80 하나만 게시합니다. 3306·8000까지 열면 DB가 인터넷에 노출됩니다.

**`eks-app/k8s/02-external-secret.yaml`** — DB 자격증명을 YAML에 적지 않습니다. External Secrets Operator가 Secrets Manager에서 읽어 쿠버네티스 Secret으로 동기화하고, 인증은 IRSA라 액세스 키가 클러스터에 저장되지 않습니다.

**`eks-app/k8s/04-nginx-ingress.yaml`** — 노드가 프라이빗 서브넷에 있어 NodePort로는 외부에서 닿을 수 없습니다. Ingress(ALB)로 노출하고 `target-type: ip` 로 파드에 직접 라우팅합니다. 파드 IP는 계속 바뀌므로 쿠버네티스가 등록·해제를 맡아야 합니다.

**`eks-cluster/cluster.yaml`** — `vpc.id` 를 명시해 기존 VPC를 재사용합니다. 지정하지 않으면 eksctl이 VPC를 새로 만들어 RDS와 다른 네트워크가 되고, 파드가 RDS Proxy에 붙지 못합니다. `Name` 태그 중복으로 `CREATE_FAILED` 가 나는 함정도 주석에 적어 두었습니다.

**`lambda/lambda-student.py`** — pymysql이 RDS Proxy의 `caching_sha2_password` 클라이언트 인증을 통과하지 못해 `1045 Access denied` 가 납니다. 프록시의 `ClientPasswordAuthType` 만 `MYSQL_NATIVE_PASSWORD` 로 내리면 클라이언트↔프록시 구간만 바뀌고 프록시↔DB는 그대로라 DB 측 보안 수준은 유지됩니다. TLS 옵션이 `ssl={"ssl":{}}` 가 아니라 `ssl_ca` 여야 한다는 점도 주석에 있습니다 — 전자는 **오류 없이 무시되어 평문 접속**이 됩니다.
...
```
/loadlist/items  ->  504   nginx 가 fastAPI 에 못 붙음
/                ->  200   기존 경로. 백엔드 장애를 감지하지 못한다

타겟     [1] healthy  [2] healthy
         [3] unhealthy  Target.ResponseCodeMismatch  codes: [502]
재기동 후 [1] unhealthy  [2] healthy
```

경로가 nginx 설정([`ec2/nginx/default.conf`](ec2/nginx/default.conf))의 `/loadlist/` 프록시 규칙과 맞물립니다. 감시 체계는 **실제로 실패시켜 봐야** 검증됩니다.
...
VPC·서브넷·보안 그룹·RDS·S3·API Gateway 는 미리 만들어져 있어야 하고, 그 ID를 `.env` 에 적습니다.
